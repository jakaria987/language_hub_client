...
const ManageClasses = () => {
    ...
    const handleApproveClass = async (id) => {
        try {
            await axios.patch(`https://final-assignment-server-jakaria987.vercel.app/classes/${id}`, { action: 'approve' });
            refetch();
    ...
    const handleDenyClass = async (id) => {
        try {
            await axios.patch(`https://final-assignment-server-jakaria987.vercel.app/classes/${id}`, { action: 'deny' });
            refetch();
            setDisabledButtons([...disabledButtons, id]);
        } catch (error) {
            console.log(error);
    ...
    const handleFeedbackSubmit = async () => {
        try {
            await axios.patch(`https://final-assignment-server-jakaria987.vercel.app/classes/${currentItemId}`, {
                action: 'feedback',
                feedback: feedback
            });
            setShowModal(false);
    ...
    return (
        <div>
            ...
            <div className="grid lg:grid-cols-2 gap-24 ml-20">
                {
                    classes.map(course =>
